...
| B2 | Post r/digitalnomadlife + LinkedIn | B | 20 May | 20 May | 30min | ✅ |
| B2 | Hilo X semanal | B | — | — | 1h/sem | Pendiente |
| B3 | Telegram contenido diario | B | 20 May | 20 May | 30min | ✅ |
| B4 | Foros viajeros (LosViajeros) | B | — | — | 2h/sem | Pendiente |
| B5 | Email outreach bloggers/agencias | B | — | — | 4h | Pendiente |
...
- **20 May: Sprint Captación día 1 completado + extras infra/ML.** Pendientes: B4 (foros), B5 (email outreach), C1-C3 (partners), D3 (referidos)
- **Extras día 1 (infra + ML):** Cron async fix (202 + background, timeout 100s → resuelto), word frequency anomaly detection (`osint_word_trends`), sentimiento integrado en `computeRiskScore()`, RLS fix en 4 tablas, sitemap fix (route handler dinámico), robots.txt limpiado, queHacer 3 países, **deploy OOM fix** (swap + NODE_OPTIONS, builds rotos desde migración a Hetzner)
...
- **Deploy fix crítico:** Hetzner CX22 (2GB RAM) se quedaba sin memoria en `npm run build`. Fix: swap 2GB + `NODE_OPTIONS=--max-old-space-size=1536` en el workflow de GitHub Actions. Sin este fix, ningún deploy funcionaba.
