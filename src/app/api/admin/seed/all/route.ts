...
export async function GET() {
  try {
    const [airports, events, travelAttrs, countryNames, openSkyBounds, disposableEmails, indices, paises] = await Promise.all([
    ...
    return NextResponse.json({ ok: true, airports, events, travelAttrs, countryNames, openSkyBounds, disposableEmails, indices, paises });
  } catch (error) {
