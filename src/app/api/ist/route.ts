...
import { GLOBAL_EVENTS } from '@/data/eventos-globales';
...
const CITY_PATTERNS: Record<string, CityPattern> = {
  barcelona: { name: 'Barcelona', code: 'barcelona', country: 'España', countryCode: 'ES', seasonMultiplier: 1.3, basePriceIndex: 1.5, events: [] },
  roma: { name: 'Roma', code: 'roma', country: 'Italia', countryCode: 'IT', seasonMultiplier: 1.4, basePriceIndex: 1.4, events: [] },
  paris: { name: 'París', code: 'paris', country: 'Francia', countryCode: 'FR', seasonMultiplier: 1.35, basePriceIndex: 1.6, events: [] },
  madrid: { name: 'Madrid', code: 'madrid', country: 'España', countryCode: 'ES', seasonMultiplier: 1.2, basePriceIndex: 1.3, events: [] },
  lisboa: { name: 'Lisboa', code: 'lisboa', country: 'Portugal', countryCode: 'PT', seasonMultiplier: 1.25, basePriceIndex: 1.2, events: [] },
  vencia: { name: 'Venecia', code: 'vencia', country: 'Italia', countryCode: 'IT', seasonMultiplier: 1.5, basePriceIndex: 1.7, events: [] },
  amsterdam: { name: 'Ámsterdam', code: 'amsterdam', country: 'Países Bajos', countryCode: 'NL', seasonMultiplier: 1.2, basePriceIndex: 1.4, events: [] },
  munich: { name: 'Múnich', code: 'munich', country: 'Alemania', countryCode: 'DE', seasonMultiplier: 1.15, basePriceIndex: 1.35, events: [] },
};
...
function calculateIST(cityCode: string, date: Date): {
  ...
  factors: { season: number; price: number; events: number; weekday: number; };
  activeEvents: { name: string; impact: number }[];
} {
  ...
  if (!city) {
    return {
      ...
      factors: { season: 35, price: 35, events: 35, weekday: 35 }, activeEvents: [],
    };
  }
  ...
  // Cargar eventos globales dinámicamente
  const globalEventsForCityAndMonth = GLOBAL_EVENTS.filter(e => 
    e.code === city.countryCode.toLowerCase() && e.month === month
  );
  
  const eventScore = globalEventsForCityAndMonth.reduce((sum, e) => sum + (e.istImpact || 15), 0) * 2;
  const activeEvents = globalEventsForCityAndMonth.map(e => ({ name: e.name, impact: e.istImpact || 15 }));
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    ...
    methodology: 'IST = 0.35×Season + 0.25×Price + 0.20×Events (Global) + 0.20×Weekday',
    note: 'Índice dinámico: incluye eventos globales (F1, Congresos, etc.).',
  });
}
