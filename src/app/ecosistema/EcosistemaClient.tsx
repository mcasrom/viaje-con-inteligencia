...
const items: Record<LayerKey, { name: string; desc: string; color: string }[]> = {
  sources: [
    { name: 'MAEC', desc: `Ministerio Exteriores España · Riesgo viaje ${TOTAL_PAISES} países`, color: 'blue' },
    { name: 'US State Dept', desc: 'Travel Advisories · Nivel numérico 1-4', color: 'indigo' },
    { name: 'UK FCDO', desc: 'Foreign Travel Advice · Nivel riesgo 1-4 (nuevo)', color: 'purple' },
  ...
  pipelines: [
    { name: 'MAEC Scraper', desc: 'Scrape exteriores.gob.es → maec_risk_history', color: 'blue' },
    { name: 'US State Dept Scraper', desc: 'Scrape travel.state.gov → external_risk', color: 'indigo' },
    { name: 'UK FCDO Scraper', desc: 'Scrape gov.uk/foreign-travel-advice → external_risk (nuevo)', color: 'purple' },
    ...
    { name: 'Social Publisher', desc: 'Telegram + Bluesky + Mastodon (rotación ES/EN)', color: 'orange' },
  ...
  storage: [
    ...
    { name: 'external_risk', desc: 'Riesgo US State Dept + UK FCDO (multi-fuente)', color: 'indigo' },
