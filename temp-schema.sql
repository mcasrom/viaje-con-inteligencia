...
CREATE POLICY "Users can view own alert preferences" ON public.alert_preferences FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can manage own alert preferences" ON public.alert_preferences FOR ALL USING (auth.uid() = user_id);
